...
%         Dimention, 4 blocks on 4th dimension
    ...
    D = dims./doms; % length of domains on each dim
    coords = index2coor(0:prod(dims)-1, dims);

    % find each coord's domain, now the coords in same domain will has the
    % same new coords
...
% function p=partitioning(dim, bs, doms)
% 	dim=dim(:)';
% 	doms=doms(:)';
% 	D = ceil(dim./doms);
% 	p = kron(coor2index(floor(index2coor(0:prod(dim)-1,dim)./D), doms), ones(bs,1));
%     assert(numel(unique(p)) <= prod(doms))
% end
